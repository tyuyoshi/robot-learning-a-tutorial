...
def main():
    ...
    predictions, targets, states, timestamps = [], [], [], []
    with torch.inference_mode():
        for batch in DataLoader(dataset, batch_size=8, shuffle=False, num_workers=0):
            ...
            predicted = post(policy.predict_action_chunk(pre(observations))[:, 0]).cpu()
    ...
    with (args.output / "predictions.csv").open("w", newline="") as output:
        ...
        writer.writerow(["timestamp"] + [f"{kind}.{name}" for kind in ("target", "predicted", "state") for name in names])
