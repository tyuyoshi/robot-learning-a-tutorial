...
2026-09-13。両腕の校正・追従とWindows経由のカメラ画像取得を確認。[対話・全つまずき・コマンド解説](so101-setup-lessons.md)に経緯と再開手順をまとめた。
...
- `max_relative_target=1` では肩・肘が動かず、`3` で動いたと報告。その後、待っても残る肘のずれが `5` の15秒試行で改善したと報告。10Hzで試験。これは現在位置からの目標差の制限で、use_degrees=Trueの回転関節は度、グリッパーは開閉率のパーセントポイント。速度制限や衝突防止ではない。全可動範囲の追従精度は未検証。
...
- Windows撮影 → ZMQ → LeRobot 0.6.1 の `ZMQCamera` で受信し、640×480の3枚に欠損がないことを目視確認。手首へ取り付け後、爪と対象物が見える画角へ調整し、初動確認に進めると判断。連続運用時のfps・遅延、アームとの同時収録、全軌道の視野は未検証。
