...
- `nvidia-smi` の CUDA 表示は 13.1。CUDA Toolkit のインストール確認や PyTorch の GPU 計算テストは未実施。
- LeRobot の学習用環境は未構築。シミュレーター、USB 接続、カメラの動作も未確認。
- ロボットは購入済み・未セットアップとの申告。機種・構成は回答待ち。
